    ...
    describe DestroyScopedAdmin do
      ...
      describe "when valid" do
        it "broadcasts :ok and removes the item" do
          ...
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admins).value.keys).to include("bar")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admins).value.keys).not_to include("foo")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admins).value.values).to include([789, 901])
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admins).value.values).not_to include([123, 456])
        end
      end
      ...
      describe "when has a constraint" do
        let(:config_helper) { create :awesome_config, organization: organization, var: :scoped_admin_foo }
        ...
        it "removes the config helper and the constraint" do
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admin_foo).constraints.count).to eq(1)
          expect { subject.call }.to broadcast(:ok)

          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admins).value.keys).not_to include("foo")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_admin_foo)).to be(nil)
        end
      end
    end
  end
