    ...
    describe DestroyScopedStyle do
      ...
      describe "when valid" do
        it "broadcasts :ok and removes the item" do
          ...
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_styles).value.keys).to include("bar")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_styles).value.keys).not_to include("foo")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_styles).value.values).to include(".body {background: blue;}")
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_styles).value.values).not_to include(".body {background: red;}")
        end
      end
      ...
      describe "when has a constraint" do
        let(:config_helper) { create :awesome_config, organization: organization, var: :scoped_style_foo }
        ...
        it "removes the config helper and the constraint" do
          expect(AwesomeConfig.find_by(organization: organization, var: :scoped_style_foo).constraints.count).to eq(1)
          expect { subject.call }.to broadcast(:ok)
