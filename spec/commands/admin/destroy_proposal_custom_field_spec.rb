    ...
    describe DestroyProposalCustomField do
      ...
      describe "when valid" do
        it "broadcasts :ok and removes the item" do
          ...
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_fields).value.keys).to include("bar")
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_fields).value.keys).not_to include("foo")
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_fields).value.values).to include('[{"type":"textarea","required":true,"label":"Bio","name":"bio"}]')
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_fields).value.values).not_to include('[{"type":"text","required":true,"label":"Age","name":"age"}]')
        end
      end
      ...
      describe "when has a constraint" do
        ...
        it "removes the config helper and the constraint" do
          ...
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_fields).value.keys).not_to include("foo")
          expect(AwesomeConfig.find_by(organization:, var: :proposal_custom_field_foo)).to be_nil
        end
      end
    end
  end
