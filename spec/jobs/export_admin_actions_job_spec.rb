...
module Decidim::DecidimAwesome
  describe ExportAdminActionsJob do
    ...
    let(:collection_ids) { Decidim::DecidimAwesome::PaperTrailVersion.role_actions.pluck(:id) }
    let(:format) { "CSV" }
    let(:ext) { "csv" }
    ...
    context "when filtered data" do
      let(:collection_ids) { PaperTrailVersion.role_actions.where(item_type: "Decidim::AssemblyUserRole").where("created_at > ?", 3.days.ago).pluck(:id) }
      let(:result) do
        [
          {
