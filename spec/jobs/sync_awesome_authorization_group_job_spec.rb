  ...
  describe SyncAwesomeAuthorizationGroupJob do
    ...
    describe "#perform" do
      context "when a member user should be granted" do
        let(:user) { create(:user, :confirmed, organization:, email: "member@example.org") }
        ...
        it "creates or updates the awesome authorization" do
          ...
          end.to change { Decidim::Authorization.find_by(user:, name: "awesome_authorization_handler")&.granted? }
            .from(nil).to(true)
        end
      end

      context "when a previously granted user is no longer a member" do
        let(:user) { create(:user, :confirmed, organization:, email: "old_member@example.org") }
        let!(:authorization) { create(:authorization, :granted, user:, name: "awesome_authorization_handler", metadata: { "groups" => { authorization_group.id.to_s => authorization_group.name } }) }
      ...
      context "when a user belongs to two groups and is removed from one" do
        let(:user) { create(:user, :confirmed, organization:, email: "shared@example.org") }
        let(:other_group) { create(:awesome_authorization_group, organization:) }
        let!(:member_in_synced_group) { create(:awesome_authorization_member, authorization_group:, email: user.email) }
        ...
        it "keeps the authorization and removes the synced group from metadata" do
          expect do
            described_class.perform_now(authorization_group.id)
          end.not_to(change { Decidim::Authorization.where(user:, name: "awesome_authorization_handler").count })
